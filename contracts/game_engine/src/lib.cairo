...
pub trait IGameEngine<TContractState> {
    ...
    fn get_leaderboard(self: @TContractState, limit: u8) -> Array<LeaderboardEntry>;
    fn check_achievements(ref self: TContractState, user: ContractAddress);
    fn get_user_achievements(self: @TContractState, user: ContractAddress) -> Array<u8>;
    fn get_achievement_info(self: @TContractState, achievement_type: u8) -> AchievementInfo;
...
/// User profile structure - Ajout de Copy trait
...
/// Leaderboard entry - Ajout de Copy trait
#[derive(Drop, Serde, starknet::Store, Copy)]
...
pub mod GameEngine {
    ...
    fn constructor(
        ...
        self.score_per_satoshi.write(1);
        self.level_threshold.write(1000);
    ...
    impl GameEngineImpl of IGameEngine<ContractState> {
        ...
        fn check_achievements(ref self: ContractState, user: ContractAddress) {
            ...
            let mut i: u8 = 0;
            while i < 10 {
                let requirements = self.achievement_requirements.read(i);
                let already_unlocked = self.user_achievements.read((user, i));
                
                if !already_unlocked && self._check_achievement_requirements(profile, requirements) {
                    self.user_achievements.write((user, i), true);
                    
                    let mut updated_profile = profile;
                    updated_profile.achievements_unlocked += 1;
                    let achievement_info = self.achievement_info.read(i);
        ...
        fn get_user_achievements(self: @ContractState, user: ContractAddress) -> Array<u8> {
            ...
            let mut i: u8 = 0;
            while i < 10 {
                if self.user_achievements.read((user, i)) {
                    result.append(i);
                }
                i += 1;
            }
    ...
    impl InternalImpl of InternalTrait {
        fn _initialize_achievements(ref self: ContractState) {
            ...
            let long_term_req = AchievementRequirements {
                min_stake_amount: 0,
                min_staking_duration: 365 * 24 * 60 * 60,
                min_score: 0,
                min_streak: 0,
            };
        ...
        fn _update_leaderboard(ref self: ContractState, user: ContractAddress) {
            ...
            let mut i: u256 = 0;
            let size: u256 = self.leaderboard_size.read();
        ...
        fn _shift_leaderboard(ref self: ContractState, start_index: u256) {
            ...
            let mut i = if size < 100 { size } else { size - 1 };
