...
pub trait IPaymentRouter<TContractState> {
    ...
    fn get_supported_chains(self: @TContractState) -> Array<ChainInfo>;
    /// Get chain info
    fn get_chain_info(self: @TContractState, chain_id: u8) -> ChainInfo;
    /// Get routing fee
    fn get_routing_fee(self: @TContractState, from_chain: u8, to_chain: u8) -> u256;
    /// Get payment status
...
pub mod PaymentRouter {
    use super::{ChainInfo, PaymentStatus, ContractAddress, get_caller_address};
    use starknet::storage::{StoragePointerReadAccess, StoragePointerWriteAccess};
    use starknet::storage::Map;
    use starknet::storage::{StorageMapReadAccess, StorageMapWriteAccess};
    ...
    impl PaymentRouterImpl of super::IPaymentRouter<ContractState> {
        ...
        fn bridge_starknet_to_bitcoin(ref self: ContractState, amount: u256, bitcoin_address: felt252) -> u256 {
            ...
            // Utiliser une adresse zéro valide pour Starknet
            ...
            let payment_status = PaymentStatus {
                ...
                to_chain: 0,   // Bitcoin
                amount: net_amount,
                recipient: zero_address, // Bitcoin address stored separately
                status: 1, // Processing
                created_at: current_time,
                completed_at: 0,
                tx_hash: bitcoin_address, // Store bitcoin_address in tx_hash temporarily
            };
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn _verify_cross_chain_proof(self: @ContractState, proof: Array<felt252>, payment: PaymentStatus) -> bool {
            ...
            // Simplified verification: just check that the proof is not empty
